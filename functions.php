...
function redirect_to_google_search() {
	if ( is_search() && ! empty( get_search_query() ) ) {
		...
		$site_url = 'site:aiccrop.com';
		wp_redirect( 'https://www.google.com/search?q=' . $site_url . '+' . $search_query );
		exit();
	}
} //redirect to google search
//add_action('template_redirect', 'redirect_to_google_search');
...
$shared_lib_basepath = 'https://aiccrop.com/wp-content/themes/Sakurairo';
$core_lib_basepath = 'https://aiccrop.com/wp-content/themes/Sakurairo';
...
function replace_jquery_cdn() {
	...
	wp_register_script('jquery', 'https://cdn.staticfile.net/jquery/3.7.1/jquery.min.js', false);
	wp_register_script('jquery-migrate', 'https://cdn.staticfile.net/jquery-migrate/3.4.1/jquery-migrate.min.js', array('jquery'));
	wp_enqueue_script('jquery');
	wp_enqueue_script('jquery-migrate');
}
add_action('wp_enqueue_scripts', 'replace_jquery_cdn', 8);
*/
...
function generate_uuid_v7() {
	$time = microtime( true ) * 10000; // 微秒时间戳
	$time = base_convert( $time, 10, 16 ); // 转换为16进制
	$time = str_pad( $time, 16, '0', STR_PAD_LEFT ); // 填充到16个字符

	$random = bin2hex( random_bytes( 8 ) ); // 生成8字节随机数
	// 组合时间戳和随机数来生成类似UUIDv7的标识符
	return sprintf(
		'%08s-%04s-7%03s-%04x-%012s',
		substr( $time, 0, 8 ),
		substr( $time, 8, 4 ),
		substr( $time, 12, 3 ),
		( hexdec( substr( $random, 0, 4 ) ) & 0x0fff ) | 0x4000,
		substr( $random, 4 )
	);
}
					...
					<div class="main shadow">
						<div class="profile">
							<a href="<?php comment_author_url(); ?>" target="_blank"
								rel="nofollow"><?php echo str_replace( 'src=', 'src="' . iro_opt( 'load_in_svg' ) . '" onerror="imgError(this,1)" data-src=', get_avatar( $comment->comment_author_email, '80', '', get_comment_author(), array( 'class' => array( 'lazyload' ) ) ) ); ?></a>
						</div>
						<div class="commentinfo">
							<section class="commeta">
								<div class="left">
									<h4 class="author"><a href="<?php comment_author_url(); ?>" target="_blank"
											rel="nofollow"><?php echo get_avatar( $comment->comment_author_email, '24', '', get_comment_author() ); ?><span
												class="bb-comment isauthor"
												title="<?php _e( 'Author', 'sakurairo' ); ?>"><?php _e( 'Blogger', 'sakurairo' ); /*博主*/ ?></span>
											<?php comment_author(); ?>
											<?php echo get_author_class( $comment->comment_author_email, $comment->user_id ); ?></a>
									</h4>
								</div>
								<?php comment_reply_link( array_merge( $args, array( 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
								<div class="right">
									<div class="info"><time
											datetime="<?php comment_date( 'Y-m-d' ); ?>"><?php echo poi_time_since( strtotime( $comment->comment_date ), true ); //comment_date(get_option('date_format'));  
	...
	function custom_html() {
		...
		?>
			<script type="text/javascript">
				...
				img.src = src
				img.addEventListener('load', afterLoaded, {
					once: true
				}) <
							? php //3 秒钟内加载不到图片也移除加载中提示
						?
							>
...
add_filter( 'comment_text_rss', 'bili_smile_filter_rss' ); //替换评论 rss 关键词
...
function sql_insert_qq_field( $comment_ID, $commmentdata ) {
	...
	update_comment_meta( $comment_ID, 'new_field_qq', $qq ); // new_field_qq 是表单 name 值，也是存储在数据库里的字段名字
}
// 后台评论中显示 qq 字段
...
function output_comments_qq_columns( $column_name, $comment_id ) {
	switch ( $column_name ) {
		case "new_field_qq":
			// 这是输出值，可以拿来在前端输出，这里已经在钩子 manage_comments_custom_column 上输出了
...
function markdown_parser( $incoming_comment ) {
	...
	if ( preg_replace( $re, 'temp', $incoming_comment['comment_content'] ) != strip_tags( preg_replace( $re, 'temp', $incoming_comment['comment_content'] ) ) ) {
		siren_ajax_comment_err( '评论只支持 Markdown 啦，见谅╮(￣▽￣)╭<br>Markdown Supported while <i class="fa-solid fa-code"></i> Forbidden' );
...
function mimvp_file_is_displayable_image( $result, $path ) {
	...
	// }
	// return $result;
	return (bool) ( $info ); // 根据文档这里需要返回一个 bool
}
	...
	?>
		<div style="margin: 0.5em 0;">
			<div class="xControl">
				...
				<span class="xTitle"><?= $atts['title'] ?></span>&nbsp;&nbsp;==>&nbsp;&nbsp;<a href="javascript:void(0)"
					class="collapseButton xButton"><span
						class="xbtn02"><?php _e( 'Expand / Collapse', 'sakurairo' ); ?></span></a>
